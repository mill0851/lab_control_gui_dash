...
class DeviceManager:
    def __init__(self, devices: dict[str,IDevice]):
    ...
    def submit_job(self, job_json):
        device_key = job_json["device"]
        method_name = job_json["method"]
        func_args = job_json["args"]
        ...
        # Submit job only if device is free
        with self._lock:
            if device.busy or device.pending:
                return {"status": "busy",
                        "data": None,
                        "message": "This device is busy",
                        "job_id": None}

            device.pending = True
            job_id = str(uuid.uuid4())
            future = self.executor.submit(self._run_device_method, device, method_name, func_args)
            self._futures[job_id] = future

        return {"status":"pending",
                "data": None,
                "message": f"Job {job_id} pending.",
                "job_id": job_id}

    def _run_device_method(self, device, method_name, args):
        try:
