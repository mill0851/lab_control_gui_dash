...
class WLM(interface.IDevice):
    ...
    def __init__(self, version=None, dll_path=None, app_path=None, autostart=True):
        # ---- Inherit Generic Device ----
        super().__init__()

        # ---- Load dll functions, they have a complex method for this ----
        self.lib=WlmDataLib()
    ...
    def open(self):
        ...
        # Open software
        try:
            self.lib.ControlWLMEx(mode,
                                  self.app_path or 0,
                                  self.version or 0,
                                  int(self._start_timeout*1E3),
                                  0)
    ...
    def close(self):
        ...
        # Close Software
        action = ECtrlMode.cCtrlWLMExit
        self.lib.ControlWLMEx(action,
                              self.app_path or 0,
                              self.version or 0,
                              int(self._start_timeout*1E3),
                              0)
    ...
    # ---- Measurement ----
    # @muxchannel
    def get_frequency(self, channel=None, error_on_invalid=True, wait=True, timeout=1.):
        """
        ...
        while True:
            try:
                freq_thz = self.lib.GetFrequencyNum(self._get_channel(channel),0.)
                #time.sleep(10) # debug
                self.busy = False
                return freq_thz
    ...
    _p_autocal_unit = interface.EnumParameterClass("autocal_unit",
                                                   {"start": EAutocalibration.cACOnceOnStart,
                                                    "meas": EAutocalibration.cACMeasurements,
                                                    "min": EAutocalibration.cACMinutes,
                                                    "hour": EAutocalibration.cACHours, "day": EAutocalibration.cACDays})
