...
def my_callback_ex(ver, mode, _intval, dblval, _res1):
    """wlmData callback function is called whenever the wavemeter produces
    measurements or state changes"""
...
try:
    ...
    sys.exit(f'{err}\nPlease check if wlmData is installed correctly!')

# Check the number of WLM server instances
if dll.GetWLMCount(0) == 0:
    sys.exit('There is no running WLM server instance.')

# Reset timestamp to TIME_START using SynchroniseWLM
TIME_START = 1000000000
dll.SynchroniseWLM(wlmConst.cSetSync, TIME_START)
...
print('Callback function removed.')
